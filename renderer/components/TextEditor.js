...
export default class TextEditor extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    // Listen for AI panel insert requests
    this._insertHandler = (e) => {
    ...
    // Listen for AI panel selection requests
    this._selectionHandler = () => {
      if (!this.editor) return;
      const sel = this.editor.getSelection();
      const text = this.editor.getModel().getValueInRange(sel);
      window.dispatchEvent(new CustomEvent('editor-selection-result', {
        detail: { text, filePath: this.props.path },
      }));
    };
  ...
  componentWillUnmount() {
    window.removeEventListener('editor-insert', this._insertHandler);
    window.removeEventListener('editor-request-selection', this._selectionHandler);
