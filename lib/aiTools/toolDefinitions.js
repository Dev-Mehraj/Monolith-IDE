...
const TOOLS = [
  ...
  {
    name: 'read_file',
    description: 'Read the full contents of a single file. Each line is prefixed with "N\\t" (its line number) for your reference only — that prefix is NOT part of the file. When quoting text for edit_file, copy only what comes after the tab on each line and never include the line-number prefix.',
    approvalRequired: false,
    parameters: {
      type: 'object',
      properties: {
  ...
  {
    name: 'edit_file',
    description: 'Replace one exact, unique occurrence of old_string with new_string in an existing file. Use this instead of write_file to change only the specific lines that need to change. old_string must match the file contents exactly, including whitespace, and must occur exactly once. Do NOT include the "N\\t" line-number prefix that read_file shows — that is for reference only and is not part of the actual file content.',
    approvalRequired: true,
    parameters: {
      type: 'object',
      properties: {
